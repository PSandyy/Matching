...
${batch.map((p, i) => `[${i}] Distributor: ${p.distributor} | Name: ${p.name} | Address: ${p.address || p.district || p.province}`).join("\n")}`;
    ...
    // IMPORTANT: Replace this URL with your backend proxy or add your API key
    // For production, create a backend endpoint that securely calls Anthropic API
        ...
        <div className="main">
          <aside className="sidebar">
            ...
            <div className="alert alert-info" style={{ fontSize: 11 }}>
              💡 Pharmacies with the same master code are duplicates matched by AI using address & Google Maps.
            </div>
          </aside>
